...
function NuskhaDetail({ route, navigation }) {
  ...
  const updateFavorite = () => {
    ...
    db.transaction((tx) => {
      tx.executeSql(
        "UPDATE Nuskha set favorite=? where id=?",
        [val, detail.id],
        (tx, results) => {
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <Entypo
          ...
          style={{ marginHorizontal: "5%", marginVertical: "2.5%" }}
          onPress={() => {
            navigation.navigate('AddRecord',{type:'Nuskha',detail:detail,multiple:multiple})
          }}
        />
      ...
      <View style={{ height: 70, width: "100%", backgroundColor: "pink" }}>
        ...
        <DetailComponent diseasedDtail={detail} surahDetail={item}/>
        )}/>
      </ScrollView>
    </View>
  );
