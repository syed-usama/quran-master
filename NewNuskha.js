...
function NewNuskha({ route, navigation }) {
  ...
  const insertData = async() => {
    db.transaction(function (tx) {
      tx.executeSql(
        "INSERT INTO Nuskha (Category,name,Description,Image,favorite,history) VALUES (?,?,?,?,?,?)",
        [
          type.Category_Name,
          disease,
          Description,
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      >
        <Text style={{ fontWeight: "bold", fontSize: 30 }}>Add New Nuskha</Text>
      </View>
      <Image
